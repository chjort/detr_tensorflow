...
def plot_results(img, boxes, labels, probs=None, colors=None, linewidth=3, text_color="yellow", text_alpha=0.5,
                 figsize=(16, 10)):
    """
    Plots the bounding boxes and labels onto an image.
    :param img: RGB image with shape [h, w, c]
    ...
    for i in range(len(labels)):
        ...
        ax.add_patch(
            plt.Rectangle((xmin, ymin), xmax - xmin, ymax - ymin, fill=False, color=color, linewidth=linewidth))
        ...
        if probs is not None:
            ...
            text = f'{label}'
        ax.text(xmin, ymin, text, fontsize=15, bbox=dict(facecolor=text_color, alpha=text_alpha))
